...
from collections import defaultdict  # 🌟 新增：用于合并时序时的字典聚合
...
def merge_time_blocks(results_list):
    """对单张图片内切出的 Block 进行向下填充时间，并按时间点聚合数据"""
    if not results_list:
        return []

    # 1. 确保按照物理切块顺序排序
    results_list = sorted(results_list, key=lambda x: x.get('_block_id', ''))

    # 2. 向下填充日期和时间 (Forward Fill)
    current_date = None
    current_time = None
    for row in results_list:
        if row.get("日期"): current_date = row["日期"]
        else: row["日期"] = current_date
            
        if row.get("时间"): current_time = row["时间"]
        else: row["时间"] = current_time
    ...
    for row in results_list:
        ...
        if primary_key not in merged_dict:
            merged_dict[primary_key] = row.copy()
        else:
            existing_row = merged_dict[primary_key]
            for key, val in row.items():
                if key in ["日期", "时间", "_block_id", "_error"]:
                    continue
            ...
            existing_row["_block_id"] = f"{existing_row['_block_id']}-{row.get('_block_id')}"
...
def process_three_columns_batch(client, slice_dir, output_json, port, img_name):
    ...
    # 🌟 核心修改 2：在写入文件之前，执行时序合并 (垂直折叠与主键填充)
    results = merge_time_blocks(results)
...
def process_single_image(img_file, final_output_dir):
    ...
    try:
        ...
        client = Client(host=f'[http://127.0.0.1](http://127.0.0.1):{port}')
